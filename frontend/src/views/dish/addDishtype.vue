...
export default class extends Vue {
  ...
  // 监听路由参数变化，确保切换不同菜品时数据正确加载
  @Watch('$route.query.id')
  onRouteIdChange(newId: any, oldId: any) {
  ...
  private async init() {
    const dishId = this.$route.query.id
    if (!dishId) {
      return
    }
    // 清空数据，避免显示旧数据
    ...
    queryDishById(dishId).then(res => {
      if (res && res.data && res.data.code === 1) {
        // 确保获取到的数据ID与当前路由参数一致
        if (String(res.data.data.id) !== String(dishId)) {
          console.warn('菜品ID不匹配，可能数据已过期')
          return
        }
        ...
        this.$message.error(res.data.msg || '获取菜品信息失败')
      }
    }).catch(err => {
